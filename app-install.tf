...
# Declaring the remote provisioner inside the resource
    provisioner "remote-exec" {
        ...
        inline = [
            "ansible-pull -U https://github.com/b53-clouddevops/ansible.git robot-pull.yml -e MODODB_ENDPOINT=${data.terraform_remote_state.db.outputs.MONGODB_ENDPOINT} -e ENV=${var.ENV} -e COMPONENT=${var.COMPONENT} -e APP_VERSION=${var.APP_VERSION}"
        ]
    }
} 
